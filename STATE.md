...
**阶段**：阶段 0 ✅ · 1 画像+召回 ✅ · 阶段 2 纠正闭环 ✅ · 阶段 3 归因+主动询问 ✅ · **阶段 4-B 周期后台 ✅**（衰减/过期/召回门控/冲突复看/趋势）· 阶段 4-A 多源感知 · **档1 摄入口闭环 ✅** · **档2 真采集器 ✅**（Win32 前台窗口采集循环 + `npm run collector` 运行器）· **公开仓加固批次 ✅（2026-07-02）**
...
## 命令
`npm run typecheck` ｜ `npm test`（117 过）｜ `npm run build` ｜ `npm run testbench` ｜ `npm run collector`
