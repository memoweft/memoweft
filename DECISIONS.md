...
**P2-旁 已落地(2026-07-18·便携包硬化:importBundle 完整 ImportPlan)**:
- 扩生成器:新增 `parity/import.json`(真 TS importBundle;6 场景 = dryRun / merge / 同库连导两次 / 非法包 / originId 撞库 / 悬空 correctsEvidenceId)。
- Python:`portable/model.py` 补 `ImportMode`/`ImportCounts`/`ImportDuplicates`/`ImportPlan`;`portable/importer.py` **从裸 SQL happy-path 重写为完整语义**——先 `validate_bundle` 门(非法整包拒写)、按 id 判重计 `duplicates`、**originId 撞库 → 标悬空 + 连带丢弃指向它的 join 行 + 告警**、**悬空 `correctsEvidenceId` 落库前置空 + 告警**、dryRun 只算不写、写入包进事务(中途失败整体回滚 + 无事务时告警)、改用 store CRUD(不再裸 SQL)。
- 验证:6 场景 ImportPlan 逐字对拍(含 errors/warnings 文案、counts、duplicates、dbState);interop 往返保真测试同步升级到新签名(幂等改验 duplicates 全计 + counts 归零 + 库不重复)。gate:**py pytest 68/68 · mypy --strict 0**;**TS shared:check 一致 · npm test 402/402**。
- 补齐了 D-0042 Phase 1c 诚实分级里点名的「dryRun/merge 明细 / 悬空处理」缺口。**待续 P2-10(§15.3 eval harness)**。

**P2-10 已落地(2026-07-18·§15.3 eval 判定层移植 + 两处 TS 发现)——Phase 2 收官**:
- **修 TS 隐患(人类批准后改)**:`bench/eval-consolidation.mjs` 原先末尾**裸调 `main()`**——任何 `import` 都会立刻触发真实全量评测(读 .env、真调 LLM、覆写 baseline)。加 `import.meta.url` main 守卫(镜像 `gen-shared-assets.mjs` 写法)+ `export` 两个纯函数 `checkStructural`/`parseYesNo`。**判定逻辑一字未改**(评测分数与历史 run 可比性不受影响)。
- 扩生成器:新增 `parity/eval-checks.json`(checkStructural 9 例覆盖 expect 各分支 / short-reply 解析覆盖与 responseAct / 三不变量 / run.error;parseYesNo 12 例含 `YESTERDAY`/`NOPE` 的 `\b` 边界)。
- Python:`evaluation.py`(JUDGE_PROMPT **逐字对齐 TS** + `JUDGE_RUNS=3` + `GIST_SCORING_VERSION=v2` + `parse_yes_no` + `check_structural` + `judge_majority` + `score_gists`(**conflict 走确定性硬判、不调 judge**)+ `run_scenario`(建 :memory: 四 store → seed → 喂 message(显式 allow_cloud_read)→ update_profile(NullRetriever)→ 收集快照))。
- 验证:判定纯函数逐位对拍(9 + 12 例)+ harness 冒烟(stub llm 真跑写路径 → 结构判定全绿)+ conflict 硬判命中/未命中两例。gate:**py pytest 73/73 · mypy --strict 0**;**TS shared:check 一致 · npm test 402/402 · bench CLI 仍正常**。
- **发现既有缺陷(非本次引入——已 `git stash` 对比 HEAD 验证原版同样失败)**:`--selftest` **7 项失败**,根因疑为 D-0036 把证据 id 改短标号 `[e1]` 后,selftest 的 mock 仍用 `uuidsIn()` 抠 UUID → 抠不到 → `support` 空 → new/conflict 分支全被跳过。**真实评测路径不受影响**(真模型照 prompt 写 `e1`)。已开独立任务修复,刻意不并入本次(动评测口径的风险须隔离)。
- 诚实分级:**判定纯函数跨语言逐位坐实**;`run_scenario`/`score_gists` 需真 LLM,跨语言只能比 §15.3 **分布**(非逐位);**真实全量 eval 执行(49 场景打真端点)需人类起 LLM 跑**——属「评测执行」非「移植」,最终终审仍是 dogfood([[eval-blind-to-scale-bugs]])。
