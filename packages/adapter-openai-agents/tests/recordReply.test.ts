/**
 * adapter-openai-agents · v0.6 会话上下文（recordAssistantReply）单元测试（D-0040）。
 *
 ...
 *   - recordFinalReply：门控（能力探测 + conversationId + 非空文本）+ 「AI 回复永不落证据、只进上下文窗口」（铁律 3a）。
 *
 * 与 adapterContract.test.ts 的 AD-1 互补：AD-1 证 persistToolOutputs 不摄助手侧 item；本文件证 recordAssistantReply
...
function msgOutputItem(text: string): RunItem {
  return { type: 'message_output_item', rawItem: { role: 'assistant', content: [{ type: 'output_text', text }] } } as unknown as RunItem;
}
function toolOutputItem(): RunItem {
  return { type: 'tool_call_output_item', output: '{"ok":true}', rawItem: { callId: 'c1' } } as unknown as RunItem;
}
...
test('finalAssistantText：finalOutput 非串 → 从最后一条 message_output_item 拼 output_text', () => {
  const result = { finalOutput: { structured: true }, newItems: [toolOutputItem(), msgOutputItem('the reply')] };
...
test('recordFinalReply：0.6 Core + conversationId + 有回复 → 调 recordAssistantReply，内容=最终回复', () => {
  ...
  const core = { recordAssistantReply: (i: { conversationId: string; content: string }) => { calls.push(i); } };
...
test('recordFinalReply：未传 conversationId → 不调（无会话上下文，行为同旧）', () => {
  let called = false;
  const core = { recordAssistantReply: () => { called = true; } };
...
test('recordFinalReply：recordAssistantReply 抛错 → 静默吞、返回 false（记忆层出错不崩对话）', () => {
  const core = { recordAssistantReply: () => { throw new Error('boom'); } };
