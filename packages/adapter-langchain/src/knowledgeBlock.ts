 ...
 * 召回注入块拼装（读路径的中性措辞层）。
 *
 * 边界（遵循 MemoWeft「Core 无头」纪律）：注入文案只搬 Core `src/pipeline/action.ts` 的
 ...
 *   provenance 进 prompt = 绕过 tier 把未获当前 tier 内建写 prompt 资格的原文提供给模型；这些字段只经 onRecall 交宿主。
 */
 ...
 *   【绝不】进 buildKnowledgeBlock / 注入 prompt（进 prompt = 绕过 tier 将受限原文暴露给云模型）——
