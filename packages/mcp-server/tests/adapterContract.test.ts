/**
 * mcp-server · adapter-kit 契约接入（adapter contract coverage）。
 *
 * 一份 kit（../../../tests/adapter-kit）覆盖两个适配器；这里是 MCP 侧的薄驱动：
 *   - 写路径经真 in-memory core + InMemoryTransport 双工，调 memoweft_ingest_user_message；
 *   - 召回呈现走 memoweft_recall 的 structuredContent（knowledge-block rendering 结构化 JSON golden）。
 * 不起真 stdio、不触网。契约断言由 runAdapterContract 产出。
 */
...
// ── recall filtering、provenance privacy、mute semantics 的 in-memory core 种子机（createMemoWeftCore + consolidation 落库，recall/mute 经 MCP tool 端到端）──
//   knowledge-block rendering 用 fakeCore 直吐夹具（锁呈现格式）；其余场景验证 contentTypes/explain 过滤与 mute 端到端落到 Core。
//   故走in-memory core:词匹配 retriever + 消化桩把夹具沉成真认知(带真 id/证据链),再经 MCP tool 召回/静音。
...
/** 建in-memory core + 种下认知(一条 spoken 证据,可选一条 tool 证据),再 updateProfile 沉成真画像。core 关由调用方负责。 */
async function seedCore(
  ...
  await core.ingestUserMessage({
    content: 'I prefer concise answers, might be learning Rust, home timezone matters.',
    subjectId: SUBJ,
  });
  if (withTool)
    await core.ingestToolResult({
      content: 'external activity log: request at 03:00 local time',
      subjectId: SUBJ,
    });
...
/** 建 server + 连好 in-memory client。core 的关闭由调用方负责（stub core 无需关）。可注入降级 logger（graceful degradation）。 */
...
const driver: AdapterDriver = {
  ...
  // user-message ingestion：走 memoweft_ingest_user_message 落一句用户原话 → 前后计数恰好 +1、spoken。
  ...
  // assistant-output exclusion：MCP 客户端驱动 —— 写 tool 只有「用户原话」与「工具返回结果」两个摄入面，
  //   无任何【助手输出】摄入 tool。没有可调的助手落库入口 → 助手消息流经产生零证据（by design）。
  async ingestAssistantTurn(_text: string): Promise<number> {
    ...
    try {
      ...
      assert.deepEqual(
        ...
        'assistant-output exclusion：写 tool 仅摄入用户原话 / 工具返回结果 + 静音一条认知，无【助手输出】摄入入口',
      );
  ...
  // tool-result-only ingestion：外部客户端调 memoweft_ingest_tool_result 存工具返回结果 → +1 tool 证据。
  //   MCP 注册面【无】摄入 assistant/tool-call 的 tool，故 LLM 的调用意图/入参无渠道落库（tool-result-only ingestion，by design）。
  async ingestToolResult(resultPayload: string, callIntent: string): Promise<ToolResultTurnResult> {
    ...
    try {
      ...
      const after = core.memory.listEvidence({});
      const added = after.filter((e) => !beforeIds.has(e.id));
      // tool-result-only ingestion：落库证据里无一条等于/含调用意图（callIntent 含 'get_weather'，result 不含）。
      const callIntentExcluded = added.every(
        (e) => e.rawContent !== callIntent && !e.rawContent.includes('get_weather'),
      );
      return {
        delta: after.length - before.length,
        sourceKind: payload.sourceKind,
  ...
  // knowledge-block rendering：memoweft_recall 的 structuredContent 即结构化呈现面。用 fake-recall core 注入夹具。
  async recallSurface(fixture: RecallFixtureItem[]): Promise<RecallSurface> {
    const fakeCore = {
      recall: async (): Promise<RecalledCognition[]> => fixture as unknown as RecalledCognition[],
    } as unknown as MemoWeftCore;
  ...
  // graceful degradation：故障 core → 读 tool（recall）。handler 兜 core.* 抛错/超时 → 降级为空召回 + isError:false（不崩、不中断），
  ...
  // recall filtering：带 contentTypes 调 memoweft_recall → in-memory core 后过滤，返回项只含请求类型（证明过滤端到端透传）。
  async recallSurfaceFiltered(
    ...
    const core = await seedCore((ev) => {
      ...
      return spoken
        ? fixture.map((f) => ({
            content: f.content,
            contentType: f.contentType ?? 'fact',
            support: [spoken.id],
          }))
        : [];
    });
  ...
  // provenance privacy：带 explain 调 memoweft_recall → 返回项带 provenance（每条含授权位）；tier privacy filter按 tier 预筛：
  ...
  async recallSurfaceExplained(fixture: RecallFixtureItem[]): Promise<RecallSurface> {
    ...
    const factContent = (fixture.find((f) => (f.contentType ?? 'fact') === 'fact') ?? fixture[0]!)
      .content;
    ...
    try {
      ...
      // 驱动内验证：provenance 包含授权位，受限项已预筛。
      const provs = raw.flatMap((h) => h.provenance ?? []);
      assert.ok(provs.length > 0, 'provenance privacy：explain 至少一条认知带 provenance 证据链');
      const restricted = provs.filter((p) => p.allowCloudRead === false);
      const allowed = provs.filter((p) => p.allowCloudRead === true);
      assert.ok(
        restricted.length > 0,
        'provenance privacy 预筛前提：种子里含一条云受限(tool, allowCloudRead=false)证据',
      );
      for (const p of restricted) {
        // tier 隐私过滤：云受限项隐去 summary（敏感原文），只保留授权位元数据与关系。
        assert.equal(
          p.summary,
          undefined,
          'provenance privacy/tier privacy filter：云受限证据的 summary 已被 tier 预筛隐去',
        );
        assert.equal(
          typeof p.allowCloudRead,
          'boolean',
          'provenance privacy：受限项仍带授权位(metadata,非敏感载荷)',
        );
        assert.equal(
          typeof p.allowInference,
          'boolean',
          'provenance privacy：受限项仍带 allowInference 授权位',
        );
      }
      for (const p of allowed) {
        assert.ok(
          typeof p.summary === 'string' && p.summary.length > 0,
          'provenance privacy：云可读证据保留 summary（未被预筛掉）',
        );
      }
  ...
  // mute semantics：经 memoweft_mute_cognition 静音某认知 → 再召回该 id 消失、其它仍在；mute 与 confidence 正交（mute semantics）。
  async muteAndRecall(fixture: RecallFixtureItem[], muteId: string): Promise<MuteAndRecallResult> {
    ...
    const { client, close } = await connect(core);
    try {
      // 真 id ↔ 夹具 id 映射（按 content 唯一）：in-memory core 生成真 id，需映回夹具 id 供契约断言。
      ...
      const mutePayload = (
        muteRes.structuredContent as {
          result: { muted: boolean; cognition: { confidence: number } | null };
        }
      ).result;
  ...
  applicability: {
    ad3: {
      status: 'applicable',
      reason:
        'memoweft_ingest_tool_result 存工具返回结果 → +1 tool 证据；无 assistant/tool-call 摄入 tool，调用意图不落库（tool-result-only ingestion，tool-result-only ingestion）',
    },
    ad5: {
      status: 'na',
      reason:
        'no model-output ingestion path：写 tool 仅收 verbatim content 且无 evidenceId 入参，模型输出不能写入证据',
    },
    ad6: {
      status: 'applicable',
      reason:
        'handler 兜 core.* 抛错/超时 → 读工具降级空召回 + isError:false，经注入 logger 记一条（降级契约）',
    },
    ad7: {
      status: 'applicable',
      reason:
        'memoweft_recall 透传 contentTypes 进 core.recall → in-memory core 后过滤，返回项只含请求类型（端到端透传）',
    },
    ad8: {
      status: 'applicable',
      reason:
        'memoweft_recall 透传 explain → provenance 带 allowCloudRead/allowInference 授权位；tier privacy filter按 tier 预筛：云受限项隐 summary、留授权位',
    },
    ad9: {
      status: 'applicable',
      reason:
        'memoweft_mute_cognition 静音一条认知 → 召回消失、其它仍在；mute 与 confidence 正交、前后相等（mute semantics）',
    },
  },
};
