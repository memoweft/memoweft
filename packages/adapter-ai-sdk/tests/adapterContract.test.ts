/**
 * adapter-ai-sdk · adapter-kit 契约接入（adapter contract coverage）。
 *
 * 一份 kit（../../../tests/adapter-kit）覆盖两个适配器；这里是 Vercel AI SDK 侧的薄驱动：
 *   - 读路径经真 in-memory core（stub LLM + 空召回器）数 evidence 增量；
 *   - 召回呈现走真 middleware.transformParams 抽出注入文本块（knowledge-block rendering 文本块 golden）。
 * 不打真模型、不触网。契约断言由 runAdapterContract 产出（本文件是 *.test.ts，node --test 直接跑）。
 */
...
// ── 离线 core（与 mcp server.test.ts 的测试配置一致：stub LLM、空召回器、:memory: 库）──
...
//   不依赖中间件内部未导出的 RecalledLike——recall filtering 据此把「透传进 onRecall 的召回对象」当断言源。
type OnRecallItems = Parameters<NonNullable<MemoWeftMiddlewareOptions['onRecall']>>[0];

// ── recall filtering/provenance privacy 离线 stub core：照 Core 门面 recall 语义（createCore.ts:359-380）──
...
const driver: AdapterDriver = {
  ...
  // user-message ingestion：一轮用户原话经 onEnd 落库 → +1 spoken。onEnd 事件携带助手输出但被忽略。
  ...
  // assistant-output exclusion：无用户原话的一轮 —— onEnd 事件仅携带助手输出。适配器只存闭包 userMessage（此处空）→ 零落库。
  //   这正是「从不读助手消息」by design：事件里的助手内容永不成为证据。
  ...
  // user-message ingestion 幂等：同一轮 stable originId，onEnd 触发多次 → put 幂等去重 → 仍一条。
  ...
  // tool-result-only ingestion：一轮 [user, assistant(tool-call 意图), tool(返回结果)] 经 persistToolResults →
  //   只落工具返回结果为 tool 证据（+1）；assistant 的 tool-call 意图/入参根本不被读（tool-result-only ingestion）。
  async ingestToolResult(resultPayload: string, callIntent: string): Promise<ToolResultTurnResult> {
    ...
    try {
          ...
          content: [
            {
              ...
              toolCallId: 'call-1',
              toolName: 'get_weather',
              output: { type: 'text', value: resultPayload },
            },
          ],
        },
      ] as ModelMessage[];
      ...
      const after = core.memory.listEvidence({});
      const added = after.filter((e) => !beforeIds.has(e.id));
      // tool-result-only ingestion：新落库的证据里，无一条含调用意图标识串（'get_weather'）。
  ...
  // knowledge-block rendering：真 middleware 注入 → 抽出前插的文本块（addToLastUserMessage 把注入块置于 content[0]）。
  async recallSurface(
    ...
    const out = await mw.transformParams!({
      type: 'generate',
      params: paramsWith('How should I phrase this?'),
      model: MODEL,
    });
  ...
  // recall filtering：带 contentTypes 调召回 → fakeRecallCore 后过滤 → 中间件把选项透传进 core.recall → onRecall 只收到匹配类型项。
  ...
  // provenance privacy：带 explain 调召回 → fakeRecallCore 逐条附 provenance → 中间件透传 explain → 经 onRecall 交宿主。
  //   surface.items 取自 onRecall 捕获对象，携带 provenance（含 allowCloudRead/allowInference 授权位）供断言。
  //   隐私：provenance 只走 onRecall，中间件的 buildKnowledgeBlock 不用它 → 绝不进注入 prompt。
  ...
  // graceful degradation：故障 core → 读路径降级。抛错/超时被 recallMiddleware catch → 原样返回 params（未注入）= 降级，
  ...
  async runWithFaultyCore(mode: FaultMode): Promise<FaultOutcome> {
    const faulty = makeFaultyCore(mode) as unknown as Parameters<
      typeof createMemoWeftMiddleware
    >[0];
  ...
  applicability: {
    ad3: {
      status: 'applicable',
      reason:
        'persistToolResults 只读 role:tool 消息的 tool-result → +1 tool 证据；assistant 的 tool-call 意图不读（tool-result-only ingestion，tool-result-only ingestion）',
    },
    ad5: {
      status: 'na',
      reason:
        'no model-output ingestion path：onEnd 事件被忽略、只存闭包 userMessage，模型输出不能写入 evidenceId',
    },
    ad6: {
      status: 'applicable',
      reason:
        'recall 抛错/超时降级为不注入（recallMiddleware withTimeout+catch），经注入 logger 记一条（降级契约）',
    },
    ad7: {
      status: 'applicable',
      reason:
        'middleware 把 opts.contentTypes 透传进 core.recall({contentTypes}) → Core 后过滤 → onRecall 只收到匹配类型项（端到端透传）',
    },
    ad8: {
      status: 'applicable',
      reason:
        'middleware 把 opts.explain 透传进 core.recall({explain}) → Core 附 provenance（含 allowCloudRead/allowInference 授权位）→ 经 onRecall 交宿主；provenance 绝不进注入 prompt（隐私加固）',
    },
    ad9: {
      status: 'na',
      reason:
        'mute semantics not applicable for ai-sdk：读适配器（RAG-as-middleware）只做召回注入、不暴露 mute 写口；mute 负反馈是受控记忆管理（memory.mute），由宿主直接调用 Core，本适配器无此写路径',
    },
  },
};
