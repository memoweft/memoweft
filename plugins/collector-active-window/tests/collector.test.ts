/**
 * 采集插件离线护栏：活动窗口采集循环（纯逻辑，不碰真 Win32、不起真定时器）+ 样本→Observation 映射。
 * 验收：连续相同合并、阈值过滤碎片、切换产出、pause 期间不采、stop 冲刷最后一段、
 * 采不到保守截断、产出不带显式授权位（下游走 observed 保守默认=不上云）、onEmit 抛错不崩；映射 content/幂等键/meta/kind。
...
test('采集循环：onEmit 抛错（如 Host 没起）→ 上报 onError，循环不崩、继续采', async () => {
  const h = makeHarness({
    onEmit: () => { throw new Error('Host 没起'); },
  });
  h.collector.start();
  await h.feed(A, 0);
