/**
 * 采集插件离线契约测试：活动窗口采集循环（纯逻辑，不碰真 Win32、不起真定时器）+ 样本→Observation 映射。
 * 验收：连续相同合并、阈值过滤碎片、切换产出、pause 期间不采、stop 冲刷最后一段、
 * 采不到保守截断、产出不带显式授权位（下游走 observed 保守默认=不进入内建云写模型 prompt）、onEmit 抛错不崩；映射 content/幂等键/meta/kind。
 * 测试策略：注入 sampler、时钟与定时器，并直接通过 tick() 驱动采样。
 */
...
import { activeWindowToObservation } from '../src/activeWindow.ts';

/** 全注入测试台架：假时钟 + 假采样队列 + no-op 定时器，feed(win, atMs) 手动驱动一拍。 */
function makeHarness(
  over: { minDurationSec?: number; onEmit?: (e: ActiveWindowEmit) => void } = {},
) {
  let nowMs = 0;
  ...
  const collector = createActiveWindowCollector({
    ...
    setIntervalFn: () => 0, // 假定时器：不实际执行，测试手动 tick
    clearIntervalFn: () => {},
  });
...
test('采集循环：产出不带显式授权位——下游 ingest 才套 observed 保守默认（不进入内建云写模型 prompt）', async () => {
  const h = makeHarness();
  h.collector.start();
  await h.feed(A, 0);
  h.setNow(40_000);
  ...
  assert.equal(
    obs.allowCloudRead,
    undefined,
    '不显式授权上云（由 config.observedDefaults 决定=false）',
  );
  assert.equal(obs.allowLocalRead, undefined);
  assert.equal(obs.allowInference, undefined);
});

test('采集循环：onEmit 抛错（如 Host 没起）→ 上报 onError，循环不崩、继续采', async () => {
  const h = makeHarness({
    onEmit: () => {
      throw new Error('Host 没起');
    },
  });
  h.collector.start();
  await h.feed(A, 0);
...
test('映射：活动窗口样本 → 标准化 Observation（content 串 + 幂等键 + meta + kind）', () => {
  const o = activeWindowToObservation({
    app: '某游戏',
    title: '战斗',
    durationSec: 3000,
  ...
  assert.ok(
    o.content.includes('某游戏') && o.content.includes('50 分钟'),
    'content 标准化（约 50 分钟）',
  );
