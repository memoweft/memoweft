...
export function sampleForegroundWindowWin32(timeoutMs = 8000): Promise<ForegroundWindow | null> {
  ...
  return new Promise((resolve) => {
    execFile(
      'powershell.exe',
      ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-EncodedCommand', PS_ENCODED],
      { timeout: timeoutMs, windowsHide: true, encoding: 'utf8' },
      (err, stdout) => {
        if (err) { resolve(null); return; }
        try {
          // 取最后一个 base64 行解码回 UTF-8 JSON。
          const line = stdout.split(/\r?\n/).map((l) => l.trim()).filter((l) => BASE64_LINE.test(l)).pop();
          if (!line) { resolve(null); return; }
          const parsed = JSON.parse(Buffer.from(line, 'base64').toString('utf8')) as { app?: unknown; title?: unknown };
          const app = String(parsed.app ?? '').trim();
          const title = String(parsed.title ?? '').trim();
          if (!app && !title) { resolve(null); return; } // 什么都认不出 → 视为取不到
