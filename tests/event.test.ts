/**
 * 事件层 + distill 测试（地图 cell 15：离线护栏；distill 用 stub LLM）。
 */
...
test('event store：put / evidenceOf / coveredEvidenceIds / remove', () => {
  ...
  try {
    const e = evt.put({ subjectId: 'owner', summary: 's', occurredAt: '2026-06-23T01:00:00.000Z', evidenceIds: ['x', 'y'] });
...
test('distill：未整理证据 → 一个事件（覆盖它们、锚到最早）；再 distill 无 pending 不调模型', async () => {
  ...
  const evt = new SqliteEventStore(':memory:');
  try {
    const a = ev.put({ subjectId: 'owner', sourceKind: 'spoken', hostId: 'h', rawContent: '我26岁', occurredAt: '2026-06-23T01:00:00.000Z' });
    const b = ev.put({ subjectId: 'owner', sourceKind: 'spoken', hostId: 'h', rawContent: '最近有点烦', occurredAt: '2026-06-23T01:05:00.000Z' });
    const stub = { callCount: 0, async chat() { this.callCount++; return '用户26岁，最近表达烦躁。'; } };

    const r = await distill('owner', { evidenceStore: ev, eventStore: evt, llm: stub });
    assert.ok(r.event);
    assert.equal(r.pendingCount, 2);
    assert.equal(r.event.summary, '用户26岁，最近表达烦躁。');
