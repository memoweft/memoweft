/**
 * observed 授权默认值下沉到 Core 的离线隐私护栏：
...
test('put observed 无显式授权 → 套 observedDefaults（local✓ / cloud✗ / infer✓）', () => {
  ...
  try {
    ...
    assert.equal(e.allowCloudRead, false, 'observed 默认不上云（隐私不变量 B 下沉 put）');
...
test('回归：spoken 无显式授权 → allowCloudRead 跟随 privacyMode（不受本次改动影响）', () => {
  ...
  try {
    ...
    assert.equal(
      e.allowCloudRead,
      false,
      'privacyMode=true → spoken 默认不上云（通用默认跟随配置）',
    );
  } finally {
    evPrivate.close();
  }
...
test('回归：inferred 无显式授权 → 走通用默认（本次约束只管 observed，inferred 不变）', () => {
  const ev = new SqliteEvidenceStore(':memory:');
  try {
