/**
 * 续聊种子（S4 会话持久化地基）：打开旧会话时把最近几轮种回工作记忆，
...
test('Conversation.seedTurns：旧会话最近几轮种回工作记忆 → 续聊带进 prompt 上下文', async () => {
  ...
  try {
    ...
    const convo = new Conversation({
      store, retriever, cognitionStore: cog, llm,
      seedTurns: [
    ...
    assert.ok(contents.some((c) => c.includes('我叫小明')), '种子里的用户话进了上下文');
    assert.ok(contents.some((c) => c.includes('你好小明')), '种子里的助手话也进了上下文');
...
test('Conversation.systemPrompt：宿主注入的人设进 system 消息（cell 9 语气/角色归宿主）', async () => {
  const store = new SqliteEvidenceStore(':memory:');
  const cog = new SqliteCognitionStore(':memory:');
  try {
    let captured: Array<{ role: string; content: string }> = [];
    const llm = {
      callCount: 0,
      async chat(messages: Array<{ role: string; content: string }>) { this.callCount++; captured = messages; return '好的。'; },
    };
    const retriever = { async indexAll() {}, async search() { return []; } };

    const convo = new Conversation({ store, retriever, cognitionStore: cog, llm, systemPrompt: '你有长期记忆，别说会忘。' });
    await convo.handle('你好');

    assert.equal(captured[0]!.role, 'system');
    assert.ok(captured[0]!.content.includes('你有长期记忆，别说会忘。'), '宿主人设进了 system 提示');
  } finally {
    store.close();
    cog.close();
  }
