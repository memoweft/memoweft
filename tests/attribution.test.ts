...
function setupScenario() {
  ...
  // observed 默认已下沉 put 层不上云；本用例把它当【云可读候选原因】用，故显式授权上云（路线 A 手动授权测试数据）。
  const eGame = ev.put({
    subjectId: 'owner', sourceKind: 'observed', hostId: 'h',
    rawContent: '游戏开到凌晨 3:30', occurredAt: '2026-06-23T03:30:00.000Z', allowCloudRead: true,
  });
...
test('M4 归因：支撑精简 —— 现象积累一堆杂证据，假设也只挂 ≤2 原因 + 1 锚点（防爆炸）', async () => {
  ...
  try {
    ...
    const eGame = ev.put({ subjectId: 'owner', sourceKind: 'observed', hostId: 'h', rawContent: '游戏开到凌晨3:30', occurredAt: '2026-06-23T03:30:00.000Z', allowCloudRead: true }); // observed 默认不上云已下沉 put；当云可读候选原因用，显式授权
...
test('M4 归因④治脑补：现象只 1 条支撑（偶发一次）→ 不归因、不调模型（不脑补）', async () => {
  ...
  try {
    ...
    ev.put({ subjectId: 'owner', sourceKind: 'observed', hostId: 'h', rawContent: '游戏开到凌晨3:30', occurredAt: '2026-06-23T03:30:00.000Z', allowCloudRead: true }); // observed 默认不上云已下沉 put；与其它归因用例一致显式授权（本例现象没攒够、本就不归因）
