/**
 * 离线护栏：归因、带证据主动询问，以及纠正闭环。
 * 全用 stub LLM，不依赖网络。验收场景：游戏到3:30(observed) → 没睡好 → 推假设 → 带证据问 → 用户否定 → 修正。
 * 归因要求现象至少有 minPhenomenonSupport 条支撑；本文件中的候选现象均挂接至少两条证据。
 */
...
function setupScenario() {
  ...
  // observed 默认已下沉 put 层不上云；本用例把它当【云可读候选原因】用，故显式授权上云（explicit authorization path 手动授权测试数据）。
  const eGame = ev.put({
    subjectId: 'owner',
    sourceKind: 'observed',
    hostId: 'h',
...
test('归因：现象 + 时间窗观察证据 → 低置信可解释假设（挂证据、可推翻）', async () => {
  ...
  try {
    ...
    assert.ok(
      h.confidence <= config.attribution.hypothesisCap,
      `低置信封顶 ≤${config.attribution.hypothesisCap}（实际 ${h.confidence}）`,
    );
    assert.ok(['candidate', 'low'].includes(h.credStatus), '假设保持非结论性低置信状态');
    assert.ok(
      cog.sourcesOf(h.id).some((l) => l.evidenceId === eGame.id),
      '挂着观察证据可溯源',
    );
  } finally {
    ev.close();
  ...
  try {
    ...
    const junk = [
      ...
      ev.put({
        ...
        hostId: 'h',
        rawContent: t,
        occurredAt: `2026-06-23T08:0${i}:00.000Z`,
      }),
    );
    const eGame = ev.put({
      subjectId: 'owner',
      ...
      occurredAt: '2026-06-23T03:30:00.000Z',
      allowCloudRead: true,
    }); // observed 默认不上云已下沉 put；当云可读候选原因用，显式授权
    cog.put({
      subjectId: 'owner',
      content: '用户昨晚没睡好',
      contentType: 'state',
      ...
      confidence: 250,
      credStatus: 'low',
      evidence: junk.map((e) => ({ evidenceId: e.id, relation: 'support' as const })),
    });
    ...
    const stub = {
      ...
      async chat() {
        this.callCount++;
        return `{"hypotheses":[{"content":"可能因为玩游戏太晚导致没睡好","based_on_evidence_ids":["${eGame.id}","${eGame.id}","fake"]}]}`;
      },
    };
    const r = await attribute('owner', { evidenceStore: ev, cognitionStore: cog, llm: stub });
    ...
    assert.ok(
      links.some((l) => l.evidenceId === eGame.id),
      '原因证据在',
    );
  } finally {
    ev.close();
...
test('归因：现象之外无可推断证据 → 不硬编假设；已归因 → 不重复', async () => {
  const { ev, cog, eGame } = setupScenario();
  try {
    let calls = 0;
    const stub = {
...
test('归因：LLM 编造不存在的证据 id → 该假设被丢弃（防自证/幻觉）', async () => {
  ...
  try {
    const stub = {
      ...
      async chat() {
        this.callCount++;
        return `{"hypotheses":[{"content":"瞎猜的","based_on_evidence_ids":["ev-不存在"]}]}`;
      },
    };
    const r = await attribute('owner', { evidenceStore: ev, cognitionStore: cog, llm: stub });
...
test('归因：禁 state→state —— 没有行为/观察证据时不拿"另一个抱怨"硬解释', async () => {
  const ev = new SqliteEvidenceStore(':memory:');
  const cog = new SqliteCognitionStore(':memory:');
  try {
    ...
    const eAnnoyed = ev.put({
      subjectId: 'owner',
      sourceKind: 'spoken',
      hostId: 'h',
      rawContent: '有点烦',
      occurredAt: '2026-06-23T08:10:00.000Z',
    });
    const eAnnoyed2 = ev.put({
      subjectId: 'owner',
      sourceKind: 'spoken',
      hostId: 'h',
      rawContent: '还是很烦',
    ...
    cog.put({
      ...
      credStatus: 'low',
      evidence: [
        { evidenceId: eAnnoyed.id, relation: 'support' },
        { evidenceId: eAnnoyed2.id, relation: 'support' },
      ],
    });
    const stub = {
      callCount: 0,
...
test('归因：一次只归因最近一个现象（maxPhenomenaPerRun=1，防爆炸）', async () => {
  const ev = new SqliteEvidenceStore(':memory:');
  const cog = new SqliteCognitionStore(':memory:');
  try {
    const eGame = ev.put({
...
test('归因防过度推断：现象只有 1 条支撑 → 不归因、不调模型', async () => {
  const ev = new SqliteEvidenceStore(':memory:');
  const cog = new SqliteCognitionStore(':memory:');
  try {
    ...
    ev.put({
      ...
      occurredAt: '2026-06-23T03:30:00.000Z',
      allowCloudRead: true,
    }); // observed 默认不上云已下沉 put；与其它归因用例一致显式授权（本例现象没攒够、本就不归因）
    cog.put({
      subjectId: 'owner',
      content: '用户觉得累',
      contentType: 'state',
      ...
      confidence: 250,
      credStatus: 'low',
      evidence: [{ evidenceId: eTired.id, relation: 'support' }],
    });
    const stub = {
      callCount: 0,
      async chat() {
        this.callCount++;
        return '{"hypotheses":[{"content":"可能熬夜导致","based_on_evidence_ids":[]}]}';
      },
    };
    const r = await attribute('owner', { evidenceStore: ev, cognitionStore: cog, llm: stub });
    assert.equal(r.consideredPhenomena, 0, '现象没攒够，根本不考察');
    assert.equal(r.hypotheses.length, 0, '偶发一次不推因果');
    assert.equal(stub.callCount, 0, '证据不足时不调用模型，也不产生无依据推断');
...
test('主动询问：低置信假设 → 带证据提问 + 把握度透明 + 标已问去重', async () => {
  const { ev, cog, eGame } = setupScenario();
  try {
    const stub = {
      callCount: 0,
    ...
    assert.ok(
      p.evidence.some((e) => e.summary.includes('3:30')),
      '带证据问（可证伪）',
    );
    assert.ok(p.confidence > 0 && p.credStatus, '向宿主公开 confidence 与 credStatus');
...
test('接回闭环：用户回答否定假设 → consolidate correct → 旧假设失效、新事实采纳', async () => {
  ...
  try {
    ...
    const eAns = ev.put({
      ...
      sourceKind: 'spoken',
      hostId: 'h',
      rawContent: '没有，那会儿只是挂机，我在和女友打电话',
    });
    const _answerEvent = evt.put({
      subjectId: 'owner',
      summary: '用户澄清：昨晚游戏只是挂机，本人在和女友打电话，没真玩',
      occurredAt: eAns.occurredAt,
      evidenceIds: [eAns.id],
    });
    ...
    const cr = await consolidate('owner', {
      eventStore: evt,
      evidenceStore: ev,
      cognitionStore: cog,
      llm: stub2,
    ...
    assert.ok(
      active.some((c) => c.content.includes('挂机')),
      '采纳用户澄清的新事实',
    );
  } finally {
    ev.close();
...
test('归因容错解析：脏 JSON（带围栏 + 前后废话）一次解出、llm 只调 1 次', async () => {
  const { ev, cog, eGame } = setupScenario();
  try {
...
test('归因解析重试：首次无花括号纯文字、二次合法 JSON → 产出假设、llm 调 2 次', async () => {
  const { ev, cog, eGame } = setupScenario();
  try {
...
test('归因解析降级：连续两次无效输出 → 空产出、不抛错', async () => {
  const { ev, cog } = setupScenario();
  try {
