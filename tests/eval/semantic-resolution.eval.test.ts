 ...
 * 断言字段落对 + 幂等 + 非法枚举收敛 null。**核心约束**：resolved_content 是解释、不是证据——
 * 永不进 consolidate 的 support 白名单；伪造 / AI-上文 evidence_id 结构性被丢弃。
 *
 * resolver 只负责 produce + store，不修改 formedBy。测试使用 stub LLM，保持完全离线。
 */
...
function seedUtterance(
  s: ReturnType<typeof openStores>,
  rawContent: string,
...
const CASES = [
  ...
  {
    name: '澄清 → elaborate/user_stated',
    preceding: '你做游戏?',
    raw: '像素风独立游戏',
    resolved: '用户澄清在做像素风独立游戏',
    res: {
  ...
  test(`resolver：${c.name}`, async () => {
    ...
    try {
      ...
      await consolidate('u', {
        ...
        semanticResolutionStore: s.semanticResolutionStore,
...
test('解析结果非证据：伪造 evidence_id 的 resolution 被丢弃；resolved_content 永不进 support 白名单', async () => {
  const s = openStores(':memory:');
  try {
    ...
    const stub = stubReturning(
      JSON.stringify({
        ...
        resolutions: [
          ...
          {
            evidence_id: 'fake-ai-context-id',
            resolved_content: 'AI 编造的命题',
            response_act: 'affirm',
          }, // 伪造 id（模拟 AI 上文冒充证据）
        ],
      }),
    ...
    // 证据 ID 白名单：伪造 evidence_id 不在 validEvidence 中，因此丢弃。
    assert.equal(
      s.semanticResolutionStore.ofEvidence('fake-ai-context-id'),
      null,
      '伪造 evidence_id 不落 resolution',
    );
    assert.ok(s.semanticResolutionStore.ofEvidence(e.id), '真证据 resolution 照常落库');
    // 支撑 ID 白名单：所形成认知的 support 链只引用真实证据 ID，不引用 resolution / resolved_content。
    const cog = out.created[0];
    assert.ok(cog, '认知已形成');
    const sources = s.cognitionStore.sourcesOf(cog.id);
    assert.ok(
      sources.length > 0 && sources.every((l) => l.evidenceId === e.id),
      'support 只引真证据，不含伪造 id / 解析内容',
    );
  } finally {
    s.close();
  }
...
test('幂等：同证据二次 consolidate 不重复落 resolution', async () => {
  ...
  try {
    ...
    assert.equal(
      s.semanticResolutionStore.forEvidenceIds([e.id]).length,
      1,
      '同证据只落一份解析（幂等）',
    );
  } finally {
    ...
    const stub = stubReturning(
      JSON.stringify({
        ...
        resolutions: [
          {
            evidence_id: e.id,
            resolved_content: '用户确认',
            response_act: 'wat',
            prompt_act: 'bogus',
            proposition_origin: 'nonsense',
            assertion_strength: 'huh',
          },
        ],
      }),
    );
...
test(' 边界：无 semanticResolutionStore → 不落解析、consolidate 行为同旧', async () => {
  const s = openStores(':memory:');
  try {
    const e = seedUtterance(s, '是', '你喜欢爬山吗?');
    const stub = stubReturning(
...
test('来源收窄：[行为观察] 证据不落解析——结构保证，不靠提示词自觉', async () => {
  ...
  try {
    ...
    s.eventStore.put({
      subjectId: 'u',
      summary: 'evt',
      occurredAt: spoken.occurredAt,
      evidenceIds: [spoken.id, observed.id],
    });
        ...
        new: [
          {
            ...
            content_type: 'preference',
            formed_by: 'confirmed',
            support_evidence_ids: [spoken.id],
          },
        ],
        resolutions: [
    ...
    assert.equal(
      s.semanticResolutionStore.ofEvidence(observed.id),
      null,
      '[行为观察] 不落解析——模型不听话也进不了表',
    );
  } finally {
    s.close();
  }
