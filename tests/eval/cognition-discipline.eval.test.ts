 ...
 * 一组稳定编号的「脚本化对话输入 → 断言认知产出」用例，便于回归结果追溯。
 *
 ...
 *
 * 测试直接调用 computeConfidence / deriveCredStatus，并使用确定性 LLM stub 与独立 store，
 * 因而不依赖网络或外部模型。
 *
...
/** 建三库 + 关闭连接的小工具：每个 consolidate 用例各开各连接（走 noopTransaction，行为同旧）。 */
function makeStores() {
...
test('EVAL-C01 有反对证据 → credStatus 命中 conflicted（暴露不消解）', () => {
  ...
  assert.equal(
    deriveCredStatus(200, 1, 'preference'),
    'conflicted',
    '低置信 preference 遇反证也标冲突',
  );
});
...
test('EVAL-C03 conflict 分支：矛盾非纠正 → 旧认知标 conflicted 且仍活跃（不删不覆盖）', async () => {
  ...
  try {
    ...
    const llm = stubLLM(
      `{"conflict":[{"cognition_id":"${old.id}","support_evidence_ids":["${e.id}"]}]}`,
    );
    ...
    assert.ok(
      s.cog.active('owner').some((c) => c.id === old.id),
      '旧认知仍活跃（不删）',
    );
  } finally {
    s.close();
...
test('EVAL-C04 conflict：两条都留、不合成一条（矛盾双方并存）', async () => {
  ...
  try {
    ...
    assert.ok(
      sources.some((l) => l.relation === 'contradict' && l.evidenceId === e.id),
      '反证以 contradict 关系挂上，不并入原文',
    );
...
test('EVAL-C05 对照组 · 显式纠正 → 允许收敛（旧失效保留、新采纳）', async () => {
  ...
  try {
    ...
    const llm = stubLLM(
      `{"correct":[{"cognition_id":"${old.id}","content":"用户现在喝咖啡，不喝茶了","content_type":"preference","formed_by":"stated","support_evidence_ids":["${e.id}"]}]}`,
    );
    const r = await consolidate('owner', {
      eventStore: s.evt,
      evidenceStore: s.ev,
    ...
    assert.equal(
      s.cog.active('owner')[0]!.credStatus === 'conflicted',
      false,
      '纠正后新认知不带冲突',
    );
  } finally {
...
test('EVAL-C06 区分纠正与矛盾：correct 旧失效、conflict 旧仍活跃（同起点两种走向）', async () => {
  ...
  try {
    ...
    const eA = a.ev.put({
      ...
      sourceKind: 'spoken',
      hostId: 'h',
      rawContent: '纠正一下，我其实不吃辣',
    });
    a.evt.put({
      subjectId: 'owner',
      summary: '用户纠正：不吃辣',
      occurredAt: eA.occurredAt,
      evidenceIds: [eA.id],
    });
    await consolidate('owner', {
      eventStore: a.evt,
      evidenceStore: a.ev,
      cognitionStore: a.cog,
      llm: stubLLM(
        `{"correct":[{"cognition_id":"${oldA.id}","content":"用户不吃辣","content_type":"preference","formed_by":"stated","support_evidence_ids":["${eA.id}"]}]}`,
      ),
    });
    ...
    b.evt.put({
      subjectId: 'owner',
      summary: '观察到点不辣的菜',
      occurredAt: eB.occurredAt,
      evidenceIds: [eB.id],
    });
    await consolidate('owner', {
      eventStore: b.evt,
      evidenceStore: b.ev,
      cognitionStore: b.cog,
      llm: stubLLM(
        `{"conflict":[{"cognition_id":"${oldB.id}","support_evidence_ids":["${eB.id}"]}]}`,
      ),
    });
...
test('EVAL-C07 conflict 没引到有效原话 → 不凭空标冲突（宁缺毋滥）', async () => {
  ...
  try {
    ...
    const llm = stubLLM(
      `{"conflict":[{"cognition_id":"${old.id}","support_evidence_ids":["ev-不存在"]}]}`,
    );
    const r = await consolidate('owner', {
      eventStore: s.evt,
      evidenceStore: s.ev,
...
test('EVAL-M02 情绪反复被多次支持 → confidence 不随支持数升为 stable 档', () => {
  ...
  assert.notEqual(
    deriveCredStatus(many, 0, 'state'),
    'stable',
    '支持 9 次仍不 stable（重复≠稳定特质）',
  );
  assert.ok(
    deriveCredStatus(many, 0, 'state') === 'low' ||
      deriveCredStatus(many, 0, 'state') === 'candidate',
    '多次支持后仍落在 low/candidate 档',
  );
});
...
test('EVAL-M04 情绪反复（讨厌→爱）经 consolidate 落库 → credStatus 落 low/candidate', async () => {
  ...
  try {
    ...
    const llm = stubLLM(
      `{"new":[{"content":"用户当前对项目感到烦躁","content_type":"state","formed_by":"stated","support_evidence_ids":["${e1.id}"]}]}`,
    );
    const r = await consolidate('owner', {
      eventStore: s.evt,
      evidenceStore: s.ev,
    ...
    assert.ok(
      c.credStatus === 'low' || c.credStatus === 'candidate',
      `情绪落低置信档（实际 ${c.credStatus}）`,
    );
...
test('EVAL-M05 同一情绪被 reinforce 多次 → 仍不升 stable（重复不越攒越像定论）', async () => {
  ...
  try {
    ...
    const llm = stubLLM(
      `{"reinforce":[{"cognition_id":"${c0.id}","support_evidence_ids":["${e1.id}"]}]}`,
    );
    const r = await consolidate('owner', {
      eventStore: s.evt,
      evidenceStore: s.ev,
...
test('EVAL-T01 不采信 LLM 自报置信：塞 confidence:999 → 产出不等于 999', async () => {
  ...
  try {
    ...
    const llm = stubLLM(
      `{"new":[{"content":"用户喜欢喝茶","content_type":"preference","formed_by":"stated","support_evidence_ids":["${e1.id}"],"confidence":999}]}`,
    );
    const r = await consolidate('owner', {
      eventStore: s.evt,
      evidenceStore: s.ev,
...
test('EVAL-T02 LLM 嘴上「非常确定」不改自算：置信由规则定、与措辞无关', async () => {
  ...
  try {
    const rp = seed(
      plain,
      `{"new":[{"content":"用户住在上海","content_type":"fact","formed_by":"stated","support_evidence_ids":["__EV__"]}]}`,
    );
    const rb = seed(
      boastful,
      `{"new":[{"content":"用户住在上海（我非常确定，confidence:999）","content_type":"fact","formed_by":"stated","support_evidence_ids":["__EV__"]}]}`,
    );
    const a = await consolidate('owner', {
      eventStore: plain.evt,
      evidenceStore: plain.ev,
      cognitionStore: plain.cog,
      llm: stubLLM(rp),
    });
    const b = await consolidate('owner', {
      eventStore: boastful.evt,
      evidenceStore: boastful.ev,
      cognitionStore: boastful.cog,
      llm: stubLLM(rb),
    });
    assert.equal(
      a.created[0]!.confidence,
      b.created[0]!.confidence,
      '带不带「非常确定」措辞，系统自算置信一致',
    );
  } finally {
    plain.close();
    boastful.close();
...
test('EVAL-T04 推测类落低置信候选、不冒充亲述事实（同支持数下 credStatus 更保守）', () => {
  ...
  const statedConf = computeConfidence({
    contentType: 'fact',
    formedBy: 'stated',
    supportCount: 1,
    contradictCount: 0,
  });
  const inferredConf = computeConfidence({
    contentType: 'fact',
    formedBy: 'inferred',
    supportCount: 1,
    contradictCount: 0,
  ...
  // contested 与 conflicted 同列 -1：都属"有反证、不参与正常档位排序"。本例两次调用的
  //   contradictCount 都是 0，取不到这两个键——补 contested 只为查表键完备（CredStatus 新增了该值），
  //   不改动本用例的任何断言与运行时行为。
  const rank = {
    candidate: 0,
    low: 1,
...
test('EVAL-T05 consolidate 缺 formed_by → 保守当 inferred（推测），不默认高置信亲述', async () => {
  ...
  try {
    ...
    const llm = stubLLM(
      `{"new":[{"content":"用户可能单身","content_type":"fact","support_evidence_ids":["${e1.id}"]}]}`,
    );
    const r = await consolidate('owner', {
      eventStore: s.evt,
      evidenceStore: s.ev,
    ...
    const statedBaseline = computeConfidence({
      contentType: 'fact',
      formedBy: 'stated',
      supportCount: 1,
      contradictCount: 0,
