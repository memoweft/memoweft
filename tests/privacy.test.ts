 ...
 * 覆盖六处：distill / consolidate / attribute + trends / proposeAsk / revisitConflicts。
...
test('filterCloudReadable：只留 allowCloudRead=true，顺序保留', () => {
  ...
  assert.deepEqual(
    filterCloudReadable(items).map((x) => x.id),
    ['a', 'c'],
  );
});

test('distill：cloud=false 的原话不进入 LLM prompt；cloud=true 的正常进入', async () => {
  ...
  try {
    ...
    // event 只覆盖真正进入 summary 的证据。cloud=true 的照常覆盖；
    //   cloud=false 的 observed（当前 cloud tier 读不到）【不算被覆盖】、留在 pending 下轮再扫——
    //   换本地模型 / 授权上云后才被补消化（旧行为把它静默标"已覆盖"又扫不到，把"行为观察"卖点架空）。
    const covered = new Set(evt.coveredEvidenceIds('owner'));
    assert.ok(covered.has(eCloud.id), 'cloud=true 证据被事件覆盖');
    assert.ok(!covered.has(eLocal.id), 'cloud=false observed 不被覆盖，保留 pending 供后续处理');
  } finally {
    ev.close();
    evt.close();
  }
...
test('consolidate：cloud=false 的原话不进 prompt、也进不了合法支撑', async () => {
  ...
  try {
    ...
    evt.put({
      subjectId: 'owner',
      summary: '一个事件',
      occurredAt: eCloud.occurredAt,
      evidenceIds: [eCloud.id, eLocal.id],
    });
    let seen = '';
    const stub = {
      callCount: 0,
    ...
    assert.ok(
      links.some((l) => l.evidenceId === eCloud.id),
      '挂了 cloud=true 证据当支撑',
    );
...
test('attribute：cloud=false 的候选原因不进入 LLM prompt', async () => {
  const ev = new SqliteEvidenceStore(':memory:');
  const cog = new SqliteCognitionStore(':memory:');
  try {
    const eSleep = ev.put({
    ...
    ev.put({
      ...
      allowCloudRead: false,
      allowInference: true,
      occurredAt: '2026-06-23T03:35:00.000Z',
    });
...
test('revisitConflicts：cloud=false 的正/反证据都不进云端提问 prompt', async () => {
  ...
  try {
    ...
    const eLocalCon = ev.put({
      subjectId: 'u',
      sourceKind: 'observed',
      hostId: 'h',
      rawContent: LOCAL2_TXT,
      allowCloudRead: false,
    });
    cog.put({
      subjectId: 'u',
