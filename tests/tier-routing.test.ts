/**
 * cloud/local tier 路由 · 离线护栏（模型路由）。
 *
 * 验收：写路径隐私关按【当前写模型 tier】决定筛哪个授权位——
 *   tier=cloud 筛 allowCloudRead（旧行为，逐字节不变）；tier=local 筛 allowLocalRead（本地模型能读 observed）。
 * 并锁三件事：① 被过滤证据保留 pending，切换 tier 后能被补消化；
 *   ② inference=false 的证据在两个 tier 都不能进入画像（distill/consolidate 三处一致）；
...
test('filterReadableByTier：cloud 筛 allowCloudRead / local 筛 allowLocalRead / 缺省 cloud', () => {
  ...
  assert.deepEqual(
    filterReadableByTier(items, 'cloud').map((x) => x.id),
    ['a', 'c'],
  );
  assert.deepEqual(
    filterReadableByTier(items, 'local').map((x) => x.id),
    ['b', 'c'],
  );
  assert.deepEqual(
    filterReadableByTier(items).map((x) => x.id),
    ['a', 'c'],
    '缺省 = cloud',
  );
});
...
test('先云后本地：cloud tier 挂起的 observed，换 local tier 后被补消化', async () => {
  const ev = new SqliteEvidenceStore(':memory:');
  const evt = new SqliteEventStore(':memory:');
  try {
    const eCloud = ev.put({
    ...
    const r1 = await distill('owner', {
      evidenceStore: ev,
      eventStore: evt,
      llm: makeStub('事件1').stub,
    });
    ...
    assert.ok(
      covered.has(eCloud.id) && !covered.has(eObs.id),
      'eCloud 覆盖、eObs 留 pending（没被静默吞）',
    );
...
test('inference 门·distill：inference=false 的证据不进 event（即便 cloud=true）', async () => {
  ...
  try {
    ...
    assert.equal(
      r.tierBlockedCount,
      0,
      'inference=false 不计入 tierBlockedCount（读得到、只是不许推理）',
    );
    const covered = new Set(evt.coveredEvidenceIds('owner'));
    assert.ok(
      covered.has(eOk.id) && !covered.has(eNoInfer.id),
      'inference=false 不被覆盖、留 pending（可重开授权后消化）',
    );
  } finally {
    ev.close();
    evt.close();
...
test('inference 门·tier=local 也拦：inference=false 的 observed 不进 event', async () => {
  ...
  try {
    ...
    assert.equal(
      r.tierBlockedCount,
      0,
      'inference=false 不算 tier 挡住（本地读得到，只是不许推理）',
    );
  } finally {
    ev.close();
    evt.close();
...
test('consolidate·tier=local：observed(cloud=false, local=true) 进 prompt 且能当合法支撑（进画像）', async () => {
  ...
  try {
    ...
    assert.ok(
      links.some((l) => l.evidenceId === eObs.id),
      'observed 成了合法支撑',
    );
  } finally {
    ev.close();
...
test('inference 门·consolidate：inference=false 的证据不进 prompt、不成支撑（cloud tier）', async () => {
  ...
  try {
    ...
    evt.put({
      subjectId: 'owner',
      summary: '一个事件',
      occurredAt: eOk.occurredAt,
      evidenceIds: [eOk.id, eNoInfer.id],
    });
    const { stub, state } = makeStub(
    ...
    assert.ok(state.seen.includes(CLOUD_TXT), 'inference=true 的进 prompt');
    assert.ok(
      !state.seen.includes(LOCAL_TXT),
      'inference=false 的证据没有进入 prompt（授权位生效）',
    );
    const created = cog.active('owner');
    assert.equal(created.length, 1, '生成 1 条认知');
    const links = cog.sourcesOf(created[0]!.id);
    assert.ok(
      links.some((l) => l.evidenceId === eOk.id),
      'inference=true 证据成支撑',
    );
