/**
 * 附和 / AI 上下文机制 —— 结构层 plumbing 与「结构无泄漏」。
 *
...
test('证据存储:precedingAiContext 落库 + 专用只读取回;Evidence 读结构永不带它（结构墙）', () => {
  ...
  try {
    ...
    // 专用只读方法取得回
    assert.equal(
      ev.precedingAiContextOf(e.id),
      '你喜欢爬山吧?',
      'precedingAiContextOf 取回 AI 上文',
    );
    // 读结构(put 返回值 / get / all)都不带该字段 —— fromRow 结构性不映射
    assert.equal(
      'precedingAiContext' in e,
      false,
      'put 返回的 Evidence 无 precedingAiContext 字段',
    );
    const got = ev.get(e.id)!;
    assert.equal(
      'precedingAiContext' in got,
      false,
      'get() 的 Evidence 无 precedingAiContext 字段',
    );
    assert.equal(
      'precedingAiContext' in ev.all()[0]!,
      false,
      'all() 的 Evidence 无 precedingAiContext 字段',
    );
...
test('迁移(缺列补):直接构造老库(无 preceding_ai_context 列)→ 构造 store 自动补列、旧行无损、可round-trip', () => {
  ...
  // 手工构造不含 preceding_ai_context 的旧版 13 列 evidence 表（不经 openStores/runMigrations）
  ...
  db.prepare(
    `INSERT INTO evidence (id, subject_id, source_kind, host_id, origin_id, occurred_at, recorded_at, raw_content, summary, allow_local_read, allow_cloud_read, allow_inference, corrects_evidence_id)
    VALUES ('old-1','u','spoken','h',NULL,'2026-01-01T00:00:00.000Z','2026-01-01T00:00:00.000Z','旧证据','旧证据',1,1,1,NULL)`,
  ).run();
  try {
    const before = db.prepare("SELECT name FROM pragma_table_info('evidence')").all() as Array<{
      name: string;
    }>;
    assert.equal(
      before.some((c) => c.name === 'preceding_ai_context'),
      false,
      '老库无该列',
    );
    // 构造 store(共享连接)→ SCHEMA 的 CREATE IF NOT EXISTS 是 no-op、migrate() 缺列补
    const ev = new SqliteEvidenceStore(db);
    const after = db.prepare("SELECT name FROM pragma_table_info('evidence')").all() as Array<{
      name: string;
    }>;
    assert.equal(
      after.some((c) => c.name === 'preceding_ai_context'),
      true,
      'migrate() 已补列',
    );
...
test('捕获:seedTurns 种入上一轮 AI → handle 存的证据挂上那句 AI 上文', async () => {
  ...
  try {
    ...
    const convo = new Conversation({
      ...
      cognitionStore: cog,
      llm,
      seedTurns: [{ role: 'assistant', content: '你喜欢爬山吧?' }],
    });
    const out = await convo.handle('是的');
    assert.equal(
      store.precedingAiContextOf(out.storedEvidence.id),
      '你喜欢爬山吧?',
      '证据挂上了上一轮 AI 那句',
    );
    assert.equal(
      'precedingAiContext' in out.storedEvidence,
      false,
      'TurnOutcome.storedEvidence 结构上不带 AI 上文（不外泄）',
    );
  } finally {
    store.close();
    cog.close();
...
test('捕获:多轮 —— 第一轮无 AI 上文(null)、第二轮挂上第一轮 AI 回复', async () => {
  ...
  try {
    ...
    const llm = {
      ...
      async chat() {
        this.callCount++;
        return `AI回复${this.callCount}`;
      },
    };
    const convo = new Conversation({ store, retriever, cognitionStore: cog, llm });
    const t1 = await convo.handle('你好');
    assert.equal(
      store.precedingAiContextOf(t1.storedEvidence.id),
      null,
      '第一轮 window 空 → 无 AI 上文',
    );
    const t2 = await convo.handle('是的');
    assert.equal(
      store.precedingAiContextOf(t2.storedEvidence.id),
      'AI回复1',
      '第二轮挂上第一轮的 AI 回复',
    );
  } finally {
...
test('注入:consolidate 的 LLM 输入带 AI 上文;AI 上文只作原话后缀、共用真证据 id、不铸独立条目', async () => {
  ...
  try {
    ...
    assert.equal(items.length, 1, '原话条目总数=1 —— AI 上文没有形成独立证据条目');
    assert.match(items[0]!, /\[e1\]/, '唯一那条条目用的是该原话的标号');
    assert.equal(
      captured.includes(e.id),
      false,
      'v7:真 UUID 不再进 prompt（改发标号，根除截断诱因）',
    );
  } finally {
    ev.close();
    evt.close();
...
test('注入隐私门：AI 上文挂在 tier 不可读的证据上 → 不进入当前模型输入', async () => {
  const ev = new SqliteEvidenceStore(':memory:');
  const evt = new SqliteEventStore(':memory:');
  try {
...
test('端到端无泄漏:会话产生带 AI 上文的证据 → listEvidence 与 exportBundle 都拿不到那句 AI 话', async () => {
  ...
  try {
    ...
    assert.equal(
      bundle.evidenceStore.precedingAiContextOf(t2.storedEvidence.id),
      '你喜欢爬山吧?',
      '底层已存 AI 上文（否则测试空转）',
    );
    ...
    for (const e of listed)
      assert.equal('precedingAiContext' in e, false, 'listEvidence 的证据不带 AI 上文字段');
    assert.equal(
      JSON.stringify(listed).includes('你喜欢爬山吧?'),
      false,
      'listEvidence 序列化里没有那句 AI 话',
    );

    // exportBundle:同样拿不到
    const exported = exportBundle('owner', bundle);
    for (const e of exported.data.evidence)
      assert.equal('precedingAiContext' in e, false, '导出证据不带 AI 上文字段');
    assert.equal(
      JSON.stringify(exported).includes('你喜欢爬山吧?'),
      false,
      '整个导出包里没有那句 AI 话',
    );
  } finally {
