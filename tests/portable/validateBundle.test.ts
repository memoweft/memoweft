...
function seedBundle(): MemoryBundle {
  ...
  try {
    ...
    s.eventStore.put({ subjectId: 'owner', summary: 'E', occurredAt: e1.occurredAt, evidenceIds: [e1.id] });
    s.cognitionStore.put({
      subjectId: 'owner', content: 'C', contentType: 'preference', formedBy: 'stated',
...
test('validateBundle：溯源引用悬空 → 致命 error', () => {
  ...
  const broken: MemoryBundle = {
    ...b,
    data: { ...b.data, cognitionEvidence: [{ cognitionId: 'no-such', evidenceId: b.data.evidence[0]!.id, relation: 'support' }] },
  };
  const r = validateBundle(broken);
  assert.equal(r.valid, false);
  assert.ok(r.errors.some((e) => e.includes('cognitionEvidence')), '报出悬空的溯源引用');
});
...
test('validateBundle：元素缺 id → 致命 error（不被 Set(undefined) 蒙混放行）', () => {
  ...
  const noId: MemoryBundle = {
    ...b,
    data: { ...b.data, evidence: b.data.evidence.map((e, i) => (i === 0 ? { ...e, id: undefined as unknown as string } : e)) },
  };
