/**
 * conflict 路径 · 真模型端到端 dogfood（REVIEW P3 提示7）。
 *
 * 背景：conflict（矛盾但非明确纠正 → 两条都留、标 conflicted、暴露不消解，见 consolidate.ts 的 SYSTEM）
 * 是最难让 LLM 稳定判对的一环。STATE 记它还没真模型 dogfood。这里搭真模型端到端脚手架。
 *
 ...
 *
 * 断言【刻意宽松】：主断言只管【管线跑通、有结构化产出】；把『判没判对 conflict』当 dogfood
...
test('conflict 端到端 · 真模型 dogfood：早睡偏好 vs 凌晨3点打游戏（矛盾非纠正）', { skip: !HAS_LLM }, async () => {
  ...
  try {
    ...
    // —— dogfood 观察点：把模型对 conflict 的判定【打印出来】，人看『判没判对』——
    ...
    console.log('原认知『喜欢早睡』credStatus:', priorAfter?.credStatus, '| 还活跃:', priorStillActive, '| 已失效:', Boolean(priorAfter?.invalidAt));
    console.log('活跃认知条数  :', active.length);
    console.log('活跃认知内容  :', active.map((c) => `(${c.contentType}/${c.credStatus}) ${c.content}`));
    console.log('判定是否命中 conflict（credStatus=conflicted 且两条都留）:', isConflicted && priorStillActive);
    console.log('（命中=理想；判成 correct/new/无操作 都记为 dogfood 观察，不算测试失败）');
    console.log('================================\n');
