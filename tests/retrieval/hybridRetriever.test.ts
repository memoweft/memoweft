/**
 * HybridRetriever 测试：使用 RRF 融合多路召回，不访问网络或系统时间。
 *
 * 覆盖：
...
test('互补召回：向量命中 A(2字中文) / 关键词命中 B(nut⊂peanut)，hybrid 同时召回 A、B', async () => {
  ...
  try {
    ...
    assert.ok(!kHits.some((x) => x.id === 'A'), '关键词臂漏掉 A（2字中文 trigram 无输出）');
...
test('配置：kCandidate 极小截断候选（rank2 的共识 doc 不进候选）', async () => {
  ...
  assert.ok(
    wideHits.some((x) => x.id === 'B'),
    'kCandidate 放大后 B 进候选被融合',
  );
...
test('扇出：indexAll 后两通道各自可召回；移除项后 hybrid 不再返回', async () => {
  ...
  try {
    ...
    assert.ok(
      (await vector.search('peanut', 5)).some((x) => x.id === 'a'),
      '向量通道可召回 a',
    );
    assert.ok(
      (await keyword.search('quantum', 5)).some((x) => x.id === 'b'),
      '关键词通道可召回 b',
    );
    // hybrid 融合后召回 a。
    assert.ok(
