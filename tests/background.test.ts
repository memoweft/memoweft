/**
 *  离线护栏：分型衰减（读时算有效置信）+ 自然过期（临时类失效、稳定类不失效）。
 * 全用注入的时间戳，不依赖真实时钟。
 */
...
test('有效置信：情绪久不提明显降，明确偏好几乎不衰', () => {
  ...
  const stateOld = effectiveConfidence(
    { confidence: 300, contentType: 'state', updatedAt: old },
    now,
  );
  const prefOld = effectiveConfidence(
    { confidence: 700, contentType: 'preference', updatedAt: old },
    now,
  );
  assert.ok(stateOld < 50, `情绪 7 天没提 → 有效置信大幅降（实际 ${stateOld}）`);
  assert.equal(prefOld, 700, '明确偏好不衰减（preference 没配半衰期）');
  // 新鲜的情绪 ≈ 原值
  const stateFresh = effectiveConfidence(
    { confidence: 300, contentType: 'state', updatedAt: now.toISOString() },
    now,
  );
