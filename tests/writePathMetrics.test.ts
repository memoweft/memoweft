...
test('写路径仪表 · 有新事件：profileSize = 预置 active 条数、promptChars > 0', async () => {
  ...
  try {
    ...
    cog.put({ subjectId: 'owner', content: '用户喜欢喝茶', contentType: 'preference', formedBy: 'stated', confidence: 600, credStatus: 'limited' });
    cog.put({ subjectId: 'owner', content: '用户今年26岁', contentType: 'fact', formedBy: 'stated', confidence: 600, credStatus: 'limited' });
    // 预置 1 个未消化事件（触发本轮真正执行整理）。
    const e1 = ev.put({ subjectId: 'owner', sourceKind: 'spoken', hostId: 'h', rawContent: '周末去爬山了' });
    evt.put({ subjectId: 'owner', summary: '用户周末去爬山', occurredAt: e1.occurredAt, evidenceIds: [e1.id] });
    ...
    assert.equal(r.created.length, 0, '空产出 → 画像零变化（D4 只观测）');
  } finally {
...
test('写路径仪表 · 经 updateProfile：result.metrics 与 consolidate 值一致', async () => {
  ...
  try {
    ...
    assert.equal(r.metrics.profileSize, r.consolidated.profileSize, 'metrics.profileSize 从 consolidate 透传');
    assert.equal(r.metrics.promptChars, r.consolidated.promptChars, 'metrics.promptChars 从 consolidate 透传');
