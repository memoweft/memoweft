/**
 * 冲突复看（阶段 4-B）：把 conflicted 认知拿出来、并排亮正反证据主动问、去重。
 */
...
test('冲突复看：conflicted 认知 → 带正反两面证据的提问 + 标已问去重', async () => {
  ...
  try {
    ...
    // 一条正常认知不该被复看
    cog.put({ subjectId: 'u', content: '用户28岁', contentType: 'fact', formedBy: 'stated', confidence: 700, credStatus: 'stable' });
    ...
    assert.ok(p.evidence.some((e) => e.summary.includes('茶')), '亮支撑侧证据');
    assert.ok((p.contradictEvidence || []).some((e) => e.summary.includes('咖啡')), '并排亮反对侧证据');
