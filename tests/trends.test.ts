/**
 * 跨会话趋势：反复出现的状态 → 规则筛频率 + LLM 归纳 → 趋势认知。
 */
...
test('趋势：窗口内状态够频 → LLM 归纳出趋势认知（ruled、挂证据）', async () => {
  ...
  try {
    ...
    const stub = {
      ...
      async chat() {
        this.callCount++;
        return `{"trends":[{"content":"用户最近这段时间持续情绪低落","based_on_evidence_ids":${JSON.stringify(ids)}}]}`;
      },
    };
...
test('趋势：同一批证据已聚过 → 不重复聚（dedup）', async () => {
  ...
  try {
    ...
    const stub = {
      ...
      async chat() {
        this.callCount++;
        return `{"trends":[{"content":"用户最近持续低落","based_on_evidence_ids":${JSON.stringify(ids)}}]}`;
      },
    };
    await aggregateTrends('u', { evidenceStore: ev, cognitionStore: cog, llm: stub }, now);
    const r2 = await aggregateTrends(
      'u',
      { evidenceStore: ev, cognitionStore: cog, llm: stub },
      now,
    );
...
test('趋势容错解析：脏 JSON（带围栏 + 前后废话）一次解出、llm 只调 1 次', async () => {
  const ev = new SqliteEvidenceStore(':memory:');
  const cog = new SqliteCognitionStore(':memory:');
  const now = new Date('2026-06-30T00:00:00.000Z');
  try {
...
test('趋势重试：首次无花括号纯文字、二次合法 JSON → 产出趋势、llm 调 2 次', async () => {
  const ev = new SqliteEvidenceStore(':memory:');
  const cog = new SqliteCognitionStore(':memory:');
  const now = new Date('2026-06-30T00:00:00.000Z');
  try {
...
test('趋势解析降级：连续两次无效输出 → 空产出、不抛错', async () => {
  const ev = new SqliteEvidenceStore(':memory:');
  const cog = new SqliteCognitionStore(':memory:');
  const now = new Date('2026-06-30T00:00:00.000Z');
  try {
