...
test(
  'activeDriver：未强制驱动时默认走零依赖的 node:sqlite',
  {
    skip: process.env.MEMOWEFT_TEST_DRIVER
      ? `已用 MEMOWEFT_TEST_DRIVER=${process.env.MEMOWEFT_TEST_DRIVER} 强制驱动，跳过默认路径断言`
      : false,
  },
  () => {
...
test(
  'better-sqlite3 驱动：开库 / exec 建表 / 位置参数写读 / changes / close 全链通',
  { skip: skipBetter },
  () => {
    ...
    try {
      ...
      const row = db.prepare('SELECT * FROM t WHERE id = ?').get('a') as
        { id: string; n: number } | undefined;
      assert.deepEqual(row, { id: 'a', n: 1 }, 'get 位置参数取回一行');

      db.prepare('INSERT INTO t (id, n) VALUES (?, ?)').run('b', 2);
      const all = db.prepare('SELECT * FROM t ORDER BY id').all() as Array<{
        id: string;
        n: number;
      }>;
...
test(
  'better-sqlite3 驱动：命名对象绑定（裸键配 $name 占位符）——与 node:sqlite 一致',
  { skip: skipBetter },
  () => {
    const DatabaseSync = makeBetterSqlite3Constructor();
    const db = new DatabaseSync(':memory:');
...
test(
  'better-sqlite3 驱动：BEGIN/COMMIT/ROLLBACK 事务语义（openStores/迁移器所依赖）',
  { skip: skipBetter },
  () => {
    const DatabaseSync = makeBetterSqlite3Constructor();
    const db = new DatabaseSync(':memory:');
