 ...
 * 纪律：
 *   - 只把【用户消息】存为证据；助手回话【不】落证据（禁止系统自证，evidence contract）。
...
export interface ConversationDeps {
  ...
  llm: LLMClient;
  /** 可注入配置（config 去单例）：不传 = 用全局单例（含召回 topK/阈值、窗口轮数）。 */
  config?: MemoWeftConfig;
  /**
   * 续聊种子（会话持久化）：打开一条旧会话时，把它最近几轮种回工作记忆，
   * 让续聊带上下文。缺省空 = 全新会话，行为同旧。只影响回话窗口，不落库、不产证据。
   */
  seedTurns?: Turn[];
  /** 宿主注入的回话人设/系统提示（public contract：语气·角色归宿主）；缺省用库内最朴素提示。 */
  systemPrompt?: string;
  /** 可注入时钟：召回衰减门控的"现在"走它；缺省真实系统时间。前进它 → 淡了的情绪衰减出局。 */
  clock?: Clock;
}

/** 召回解释：一条召回认知背后的一条支撑/反证证据简报，供宿主看"这条记忆建立在什么上"（可追溯卖点）。
 *  带 allowCloudRead/allowInference 授权位（对齐 buildMemoryGraph 惯例）：summary 是证据【原文】（可能比派生认知更敏感、
 *  含默认不进入内建云写模型 prompt 的 observed/tool），宿主把 provenance 转发云模型前应据此按 tier 自筛——库不自动喂云，DTO 附授权位让宿主筛得了。 */
...
export interface RecalledCognition extends RelevantCognition {
  score: number;
  /** 认知 id；由共享召回函数返回，供管理与诊断反查。可选以兼容旧构造。 */
  id?: string;
  /** 认知类型：召回结果带回，供宿主看类型 + core.recall 的 contentTypes 过滤）。可选以兼容旧构造处。 */
  contentType?: ContentType;
  /** 召回解释：仅在 core.recall({ explain: true }) 时带；本条认知的支撑/反证证据链（可追溯，带授权位供宿主自筛）。 */
...
export class Conversation {
  ...
  async handle(userMsg: string, opts: PerceiveOptions = {}): Promise<TurnOutcome> {
    ...
    //   让缺少直接上下文的回应（例如“AI：你喜欢爬山吧？用户：是的”）也能进入消化流程，形成 confirmed 认知。
