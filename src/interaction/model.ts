 ...
 *     只提供语义环境，不生成 Cognition，也不作为证据摄入；禁收 system prompt / 隐藏提示 /
 *     chain-of-thought / 记忆注入 / 内部工具入参。
 *   - SemanticResolution:一条证据的【语义解析】(这句是肯定/否定/选择/含糊、在回应谁提出的什么命题)。
 *     是【解释结果、不是新的事实来源】; 起由 resolver 产出, 只建表结构。
 *
 * 证据边界：两者承载的 AI 可见文本 / 解析结果**永不进 consolidate 的 support 白名单，也不给证据 id**——
 *   与「是否进便携包」正交(进包是数据迁移,不等于内容变证据)。
 */

/** 用户回应的言语行为（由 v0.6 resolver 判定）。 */
export type ResponseAct = 'affirm' | 'negate' | 'select' | 'elaborate' | 'ask' | 'none' | 'other';
/** AI 上一句(命题)的言语行为。 */
export type PromptAct = 'propose' | 'ask' | 'state' | 'none' | 'other';
/** 命题来源:用户主动提出 vs AI 提出。附和的关键——AI 提的、用户点头 → confirmed,不成 stated(v0.6  用)。 */
