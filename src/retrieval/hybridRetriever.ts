 ...
 *
 * RRF 的用意（对 §14.6 消融解读）：doc 只要在多个通道都靠前，其 RRF 分就叠加，故"两臂都点头"的
 * doc 会压过"单臂 rank1"的 doc——这正是词面臂（keyword）与语义臂（vector）互补召回的融合点。
 *
 * 确定性：纯融合逻辑，无系统时间、无网络、无随机——rank 只取通道返回的**自身顺序**（不按 score 重排，
 * 各通道已降序），平票按"首次出现顺序"稳定裁决；故同输入恒同输出。
 *
 * ⚠ 本类**不导出到 src/index.ts**（§14.4a：公共 API 冻结面不动，api:check 保持绿）。
 */
import type { Retriever, RetrievalHit } from './retriever.ts';

/** 默认每通道取的候选数（§14.4）：融合前各通道各取前 kCandidate 个。 */
const DEFAULT_K_CANDIDATE = 50;
/** 默认 RRF 平滑常数（§14.4，经典取 60）：压低头部名次的边际权重，让"多通道共识"胜过"单通道极靠前"。 */
...
export class HybridRetriever implements Retriever {
  ...
  async search(query: string, topK: number): Promise<RetrievalHit[]> {
    ...
    // 与通道解析先后无关 → 融合结果确定。
    const perChannel = await Promise.all(this.channels.map((ch) => ch.search(query, this.kCandidate)));
