/**
 * 证据层数据模型：证据保存来源材料，只存“原料”，不存判断。
 *
 * 字段全是"关于证据的事实"——来源、时间、授权、幂等、纠正指向。
 * 判断类（置信度 / 可信状态 / 适用范围）不在这里，属+ 的 cognition 层。
 *
 * 双时态（借 Graphiti）：occurredAt = 事情发生时；recordedAt = MemoWeft 存入时。
 */

/** 来源种类与形成方式的置信层级：亲口 > 推测 > 观察/工具。
 *  'tool' = 工具执行的【返回结果】（外部客观数据）；助手生成的调用意图/入参不得作为证据摄入。 */
 ...
 *   spoken/inferred 走通用默认（evidenceDefaults + cloud 跟随 privacyMode，决定内建云写模型 prompt 是否可选）。显式传值永远优先。
 */
export interface EvidenceInput {
  ...
  correctsEvidenceId?: string | null;
  /**
   * 上一轮 AI 回复的原文，用于解析附和语境；只写入专用上下文列，不作为 Evidence 字段读回。
   * 用途:孤儿回应("AI:你喜欢爬山吧? 用户:是的")的信息只藏在 AI 那句里;把它作为【只读上下文】
   *   注入 distill/consolidate，让附和产得出 `confirmed` 认知。**永不成为证据，也不给证据 id**。
