/**
 * 写路径一键更新：把"未整理"的近期对话沉淀成事件，从全部事件重算画像，
 * 再对新现象做归因，并重建召回索引（让新画像/假设马上能被回话召回）。
 *
 * 解决 UX 缺口：distill / consolidate / 归因 / 索引本是多步，合成一个；宿主只调这一个。
 * 归因自动并进：consolidate 出新 state 现象后顺带跑 attribute，假设直接进画像；
 *   "是否开口问" 仍独立——MemoWeft 给理解、宿主定表达（public contract）。
...
export interface UpdateProfileDeps {
  ...
  /** 语义解析 store（可选）：透传给 consolidate 落 semantic_resolution；不接 = 不落解析。 */
  semanticResolutionStore?: SemanticResolutionStore;
  retriever: Retriever;
  llm: LLMClient;
  /** 事务器（可选）：接了共享连接就传它，consolidate 的写入会原子化（中途失败时整段回滚）。见 store/openStores.ts。
   *  注意只作用于 consolidate 内部那段【同步】写——索引重建是读路径优化、故意放在事务外（失败不回滚画像）。 */
  transaction?: Transaction;
  /** 可注入配置（配置去单例）：不传 = 用全局单例；传了则透传给 consolidate / attribute。 */
...
export async function updateProfile(
  ...
  const consolidated = await consolidate(subjectId, {
    ...
    semanticResolutionStore: deps.semanticResolutionStore, // v0.6 ：落语义解析（缺省 undefined = 不落）
    llm: deps.llm,
  ...
  // 索引是读路径优化（runtime boundary）——嵌入器挂了也不该让已落库的画像更新失败（呼应 conversation 的"召回失败不挡"）。
  // 索引排除 muted 项：静音认知仍 active（继续参与 consolidation/attribute 与画像演化），
  ...
  return {
    ...
    indexed,
    indexError,
    // 写路径仪表（runtime metric）：从 consolidate 结果原样透传，方便落盘处不用钻进 consolidated 里挖。
