...
Generated assets that the **Python port** (`py/`, 1.3 · D-0042) loads so both languages read **one source of truth** instead of hand-copying (hand-copying drifts — D-0042).

**The TypeScript source is authoritative.** These files are *generated* from it by [`scripts/gen-shared-assets.mjs`](../scripts/gen-shared-assets.mjs), which imports the real TS functions/constants. Do not hand-edit them.
...
The `parity/*.json` files are **`{input, expected}` fixtures**: the Python port runs its own reimplementation on each `input` and asserts it produces `expected` — bit-exact cross-language verification (porting *is* the test). `config-constants.json` and `prompts.json` are loaded directly as the single source.

**Three parity killers to reproduce exactly** (see D-0042):
