...
PR 正文写清 **改了什么 / 为什么 / 怎么验的（贴三绿输出，比如 `pass 54 fail 0`）**。CI 复跑三绿作为合并门；人 review 后合入。**禁止 `--no-verify`、禁止跳过测试。**
...
- 有了 CI，README 里「tests 54 passing」的徽章才名副其实。首版若还没挂 CI，先用 shields.io 静态徽章手写数字，**别挂假的动态徽章**。
