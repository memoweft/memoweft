...
| 要求 | 说明 |
| --- | --- |
| **Node ≥ 24** | 库直接跑 `.ts`、用 Node 内置 `node:sqlite`（作者实测 **v24**；CI 也锁 24）。`node:sqlite` 到 24 才转正、原生跑 `.ts` 也到 24 才稳定，低于 24 会报找不到 `node:sqlite` 或跑不了 `.ts`、或打实验警告。用 `node --version` 确认。 |
...
npm test            # 66 个测试全过（作者实测 66 passing）
...
A：Node 版本太低。升级到 **≥ 24**（作者实测 v24，CI 也锁 24）。`node:sqlite` 与原生 `.ts` 执行都需要新版本 Node。
