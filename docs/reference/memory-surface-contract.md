...
18. **`RecallInput`** — stable: `query` + `subjectId?` + **`explain?: boolean`** (D-0021: `true` → each recalled cognition carries its supporting `provenance` evidence chain; additive, default off = unchanged). Basis `:75-78`.
...
23. **`RecalledCognition`** — stable (`recall`/`TurnOutcome.recall` item): `RelevantCognition + score + id?` + **`provenance?: RecalledEvidence[]`** (D-0021, only when recalled via `recall({ explain: true })`; `RecalledEvidence = { evidenceId; relation; summary; sourceKind; allowCloudRead; allowInference }` — the supporting/contradicting evidence brief **with authorization flags** (aligned with `buildMemoryGraph`) so the host can tier-filter `provenance` before forwarding it to a cloud model; host-facing, for traceability; additive). Basis `src/pipeline/conversation.ts:38-42`.
