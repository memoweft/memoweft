...
每层"负责什么 / 不负责什么"与当前归位现状见 [boundaries.md](./boundaries.md)；实施路线见 [架构归位路线.md](./架构归位路线.md)。
...
**边界**：库只定义"观察怎么进来 + 怎么授权"，**不在库里写"怎么从操作系统抓"**——那是外挂采集器的活（`activeWindow.ts` 只留骨架）。这保证库自身干净、可移植。8-A 已临时把真实采集（`activeWindowCollector.ts` / `win32Foreground.ts`）放进库内（**不从主入口导出、标 experimental**），待迁 `plugins/collector-active-window/`，见 [boundaries.md](./boundaries.md) §4.1。
