 ...
 * Host 聊天历史落盘（Host 自建）。
 *
 * 这是 Host 职责的持久化编排：Host 自己记"每轮谁说了什么"，
 * 不依赖 Core 的 RunLogger 内幕格式（那是调试用、字段随内幕演进）。够用即从简：
 * 一条对话 = 一个 .jsonl 文件，每行一条 {role, content, ts}，追加写、顺序读回。
 *
 * 多对话时，一条对话一个 JSONL 文件（文件名含 conversationId）。会话册（列表/归档）是
 *   【Host 自己的持久数据】，不从 Core 掏——Core 的 conversations Map 只是活跃实例窗口缓存、
 *   故意不暴露枚举。多对话（列表/新建/切换/续聊/归档）全在这一层文件系统编排。
 *
 * 归档 = 软移除：给 jsonl 加 `.archived` 后缀，数据不删且可恢复。
 * 列表默认不列已归档会话；archive 只改后缀，内容原样保留。
 *
 * UTF-8 护栏：读写均显式使用 utf-8，避免中文内容乱码。
...
export function createChatHistory(dir: string): ChatHistory {
  ...
  const fileFor = (conversationId: string): string =>
    join(dir, `${sanitizeId(conversationId)}.jsonl`);
  ...
  return {
    append(conversationId, turn) {
      ...
      //   先把归档恢复为活跃状态，续写在同一份历史上：
      //   ① 别新建空活跃文件把归档历史遮蔽/分叉（open 已归档对话再聊，历史不隐身）；
      //   ② 之后再次归档时，renameSync 不会覆盖旧归档并丢失历史。
    ...
    list(opts) {
      ...
      for (const name of names) {
        ...
        const id = archived
          ? name.slice(0, -'.jsonl.archived'.length)
