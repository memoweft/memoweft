/**
 * experience-plain · 普通助手体验插件。
 *
 * 调性：高效、真诚、有长期记忆但不过度拟人的通用助手。它是 MemoWeft 作为
 *   "通用用户认知框架、而非某个角色专用记忆库"的第二个体验证明（对照 experience-xingyao）。
 *
 * systemPrompt 沿用 server.ts 原来硬编码的 REPLY_PERSONA：
 *   素提示下大模型会露出出厂反射「我不保留记忆、聊完就忘」，正好否定 MemoWeft 的价值，
 *   所以要显式告诉它"背后有跨对话的记忆层"。语气归宿主这一层，Core 本体不拟人。
 *
 * 普通助手不拟人过度、不自称有情感、不说"真正理解你"。它只是一个
...
/** 普通助手体验插件，提供 experience 元数据与 systemPrompt。 */
