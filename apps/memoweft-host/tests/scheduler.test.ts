/**
 * 后台画像调度器 scheduler：验证用户“立即整理”和新增理解信号。
 * 注入假 updateProfile，不依赖网络/模型/库——只验调度器自身的透传/单飞/清计数逻辑。
 *
 * 覆盖三项关键行为：
 *   ① refreshNow 把 consolidated.created 转成 lastUpdate.newCognitions（只含 id/content/credStatus）；
...
test('refreshNow：把 created 转成 lastUpdate.newCognitions（只含 id/content/credStatus）', async () => {
  const created = [
