...
### 阶段 1(建设)
- **1.1 事件类型(`event` ContentType,第 9 型)** —— 打头。核心库小特性,dogfood 驱动(填 fact/state 之间"一次性事件"缺口,见 D-0019);落地走 D-xxxx + api-freeze + 语料 + 提示词 bump + §15.2 重跑。
- **1.2 适配器规模化** —— adapter-kit 已被 6 个适配器证明,增量铺开更多官方适配器。
...
### 阶段 2(打磨 · dogfood 驱动)
- 人类在 **weftmate 上实际使用一段时间** → 拿真实使用暴露的问题回头**优化/完善 memoweft**。
